module PartTwo exposing (minimumCombinedSteps, solve)

import Path exposing (Path)


lengthFrom : Path.Line -> Path.Point -> Maybe Int
lengthFrom l ( px, py ) =
    let
        ( lx, ly ) =
            Path.endof l
...
minimumCombinedSteps : Path -> Path -> Int
minimumCombinedSteps wirea wireb =
    Path.combine wirea wireb
        |> List.foldl
            (\( linea, lineb ) dists ->
                case Path.intersection ( linea, lineb ) of
                    Just ( 0, 0 ) ->
                        dists
                    ...
                    Just point ->
                        let
                            distToLinea =
                                Path.distanceTo linea wirea

                            distToLineb =
                                Path.distanceTo lineb wireb

                            distToPointa =
                                Maybe.map2 (-) distToLinea (lengthFrom linea point)

                            distToPointb =
                                Maybe.map2 (-) distToLineb (lengthFrom lineb point)
                        in
                        Maybe.map2 (+) distToPointa distToPointb
                            |> Maybe.map (\dist -> dist :: dists)
                            |> Maybe.withDefault dists
